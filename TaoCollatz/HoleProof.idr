...
||| the exact affine relation `2^{S_n(x)} * Syr^n(x) = 3^n * x + c`.
public export
...
||| Step 3 -- the exact affine backbone of the Syracuse dynamics: for every odd
||| `x` and every `n`, there is a nonnegative correction `c` with
||| `2^{S_n(x)} * Syr^n(x) = 3^n * x + c`.  (True: one Syracuse step obeys
||| `2^{a} * Syr(z) = 3 * z + 1`, and iterating gives this closed form.)
public export
...
||| Step 4 -- the large-deviation / drift heart, in density form: for every
||| height `f -> infinity` there is a density-one set of odd starts `y` such that
||| the running valuation sum reaches the drift rate `8/5` at some time at least
||| `f y`, i.e. there is `n >= f y` with `8 n <= 5 * S_n(y)` (so `S_n(y) >=
||| (8/5) n`).  (True: this is the concentration of the 2-adic valuation sum
||| about its mean `>= 8/5`, the analytic core of Proposition 1.9.)
public export
...
||| Step 5 -- contraction dominates growth for typical points, in density form:
||| for every `f -> infinity` there is a density-one set of odd starts `y` on
||| which the contraction eventually beats the growth by the factor `f y`, i.e.
||| some `n` has `3^n * f(y) <= 2^{S_n(y)}`.  (True: combine Step 2 with the
||| drift of Step 4, since `2^{(8/5)n} = (2^8)^{n/5} >= (3^5)^{n/5} = 3^n`.)
public export
...
||| Step 6 -- typical descent below the starting value, in density form: a
||| density-one set of odd starts each of whose Syracuse orbit reaches, at some
||| time, a value at or below the start.  (True: combine the exact dynamics of
||| Step 3 with the domination of Step 5 -- since `Syr^n(x) = (3^n x + c) /
||| 2^{S_n}` and `2^{S_n}` outgrows `3^n`, the orbit falls back below `x`.)
public export
...
||| Step 7 -- density-one Syracuse first passage on the *odd* domain: for every
||| height `f -> infinity`, almost every odd `y` has a Syracuse orbit that
||| eventually drops below `f y`.  (True: iterate the single-scale descent of
||| Step 6 -- the descent below the start compounds, and a diagonal argument over
||| the growing height `f` upgrades "below the start" to "below `f`".)
public export
